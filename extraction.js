...
const DEFAULT_EXAMPLE = 'https://www.ncbi.nlm.nih.gov/nuccore/?term=blaNDM';
...
async function go(urlOverride) {
  ...
  urlInput.value = u;
  showWebview();
  try { await window.api.syncExtractionProxy(); } catch (e) {}   // 导航前按当前加速器状态刷新代理,避免空白
  wv.loadURL(u);
}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  wv.addEventListener('did-navigate', (e) => { urlInput.value = e.url; showWebview(); });
  wv.addEventListener('did-navigate-in-page', (e) => { if (e.isMainFrame) urlInput.value = e.url; });
  wv.addEventListener('did-fail-load', (e) => {
    if (e.errorCode && e.errorCode !== -3) {
      toast('页面加载失败: ' + (e.errorDescription || e.errorCode) + '（境外站点请确认加速器已开启）');
    }
  });
